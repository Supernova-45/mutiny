...
function Viewer({structure,surface,residue,bound,onReady,onPick,resetKey,focus,ghost,evidence,restore}:{structure:Structure;surface:boolean;residue:number;bound:boolean;onReady:(v:mol.GLViewer)=>void;onPick:(n:number)=>void;resetKey:number;focus:boolean;ghost:boolean;evidence:HhatEvidence|null;restore:number[]|null}) {
  ...
  useEffect(()=>{
    ...
    try {
      const v=mol.createViewer(element,{backgroundColor:'#f3f1ec',antialias:true});
      viewer.current=v;v.setProjection('orthographic');initialView.current=v.getView();ready.current(v);
      const resize=new ResizeObserver(()=>{v.resize();v.render()});resize.observe(element);
      return()=>{resize.disconnect();v.spin(false);v.clear();element.replaceChildren();viewer.current=null};
  ...
  useEffect(()=>{
    ...
    Promise.all([load(structure.id),ghost&&!bound?load(ghostId):Promise.resolve(null)]).then(async([text,ghostText])=>{
      ...
      if(focus) {
        v.setStyle({chain:'A'},{cartoon:{color:color.hla,opacity:.22}});
        if(bound)for(const chain of ['D','E'])v.setStyle({chain},{cartoon:{color:color.tcr,opacity:.25}});
        v.addStyle({chain:'C',resi:6},{stick:{radius:.27,color:'#497775'},sphere:{scale:.23,color:'#497775'}});
        if(bound) {
          v.setStyle({chain:'D',resi:100},{stick:{radius:.25,color:color.tcr},sphere:{scale:.22,color:color.tcr}});
          const contact=evidence?.contacts.find(c=>c.state===structure.id);
          if(contact){
            const xyz=(p:number[])=>({x:p[0],y:p[1],z:p[2]});
            v.addLine({start:xyz(contact.start),end:xyz(contact.end),color:'#555b57',dashed:true,linewidth:2});
            v.addLabel(`${contact.value.toFixed(2)} Å`,{position:xyz(contact.start.map((v,i)=>(v+contact.end[i])/2)),fontColor:'#333b37',backgroundColor:'#f3f1ec',backgroundOpacity:.9,fontSize:12,borderThickness:0,inFront:true});
          }
        }
      }
      ...
      if(camera.current && resetKey===lastReset.current) v.setView(camera.current);
        ...
        v.zoomTo({model:0,chain:'C'});v.zoom(.66);
        if(bound){v.rotate(62,'x');v.zoom(.7)}
        if(focus){
          if(bound){v.zoomTo({model:0,or:[{chain:'C',resi:6},{chain:'D',resi:100}]});v.zoom(1.1)}
