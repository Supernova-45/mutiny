...
import { ArrowRight, ArrowUpRight, Atom, BookOpen, Check, CircleHelp, Download, Eye, EyeOff, Github, RotateCcw, X } from 'lucide-react';
    ...
    {scene==='atlas'?<main className="atlas-page">
      ...
      <div className="atlas-layout">
        ...
        <section className="cohort-panel" aria-label="Cancer vaccine targets">
          ...
          <div className="atlas-footnote"><span><i className="status-ring"/>{lens==='published'?'':`${data.targets.filter(t=>t.comparisonEligible).length} compared · dimmed targets excluded`}</span><button className="text-button" onClick={()=>{setLens('published');setReveal(false);setSeed(20260909)}}><RotateCcw size={12}/> Reset</button></div>
        </section>
        <aside className="target-inspector">
          <div className="panel-heading"><span>VACCINE TARGET</span><span className="inspector-coordinate">PT {cleanPatient(patient)} / {selected.number}</span></div>
          <AnimatePresence mode="wait"><motion.div key={selected.id} initial={{opacity:0,y:5}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.12}}>
            <div className={`target-status ${reveal?selected.outcome:'concealed'}`}><i/>{reveal?names[selected.outcome]:''}</div>
          ...
          <button className="molecule-link" onClick={()=>setScene('molecule')}><span>Recognition in 3D</span><ArrowUpRight size={19}/></button>
        </aside>
      </div>
