...
function Evidence({data,onClose}:{data:Cohort;onClose:()=>void}) {
  ...
  return <div className="modal-backdrop" onClick={onClose}><section className="evidence" role="dialog" aria-modal="true" aria-label="Evidence and methods" onClick={e=>e.stopPropagation()}>
    ...
    <span className="eyebrow">THE RECORD BEHIND THE VIEW</span><h2>Evidence, always<br/>within reach.</h2>
...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="header-actions"><span className="live-label"><i/> AN OPEN RESEARCH ATLAS</span><button className="icon-button" title="Evidence and methods" aria-label="Evidence and methods" onClick={()=>setEvidence(true)}><BookOpen size={18}/></button><a className="icon-button github" href="https://github.com/Supernova-45/callback" target="_blank" rel="noreferrer" aria-label="GitHub repository"><Github size={18}/></a></div>
    </header>
    {scene==='atlas'?<main className="atlas-page">
      <section className="intro-bar"><div><div className="eyebrow"><span className="section-index">01 /</span> CANCER VACCINES, TARGET BY TARGET</div><h1>A mutation is a message.<br/><em>Does the immune system hear it?</em></h1></div><div className="intro-note"><span className="tiny-cross">✳</span><p>Real vaccine targets.<br/>Measured T-cell responses.<br/>A closer look at recognition.</p></div></section>
      <div className="atlas-layout">
        <aside className="control-rail">
          <div className="study-label">PANCREATIC CANCER <span>2023</span></div>
          <div className="big-count"><strong>232</strong><span>targets selected<br/>for vaccination</span></div>
          <div className="rail-divider"/>
          <div className="rail-kicker">01 &nbsp; CHOOSE A LENS</div>
          ...
          <p className="lens-description">{lens==='published'?'The target order in the published supplement.':lens==='esm'?'Learned protein-sequence preference. More negative first.':lens==='binding'?'Predicted class-I affinity. Lower nM first.':`Same targets. A new order. Seed ${seed}.`}</p>
          {lens!=='published'&&<div className="scope-note">Same eligible targets.<br/>Ordered within each patient.<span>Response class unresolved.</span></div>}
          {!hasModels&&<div className="scope-note">Model results not yet available.</div>}
          <div className="rail-kicker second-kicker">02 &nbsp; LOOK AT THE RESPONSE</div>
          ...
          <button className="text-button evidence-trigger" onClick={()=>setEvidence(true)}><CircleHelp size={14}/> What does a response mean?</button>
        </aside>
        <section className="cohort-panel" aria-label="Vaccine target atlas">
          <div className="panel-heading"><span>16 INDIVIDUAL VACCINES</span><span className="panel-hint">ONE MARK = ONE TARGET</span></div>
          <div className="cohort-axis"><span>PATIENT</span><span>{lens==='published'?'PUBLISHED ORDER':'EARLIER IN ORDER'} <ArrowRight size={12}/> {lens==='published'?'':'LATER'}</span><span>{reveal?'DETECTED':'TARGETS'}</span></div>
          ...
          <div className="atlas-footnote"><span><i className="status-ring"/>{lens==='published'?'Already selected and administered':`${data.targets.filter(t=>t.comparisonEligible).length} jointly eligible · dimmed marks outside comparison`}</span><button className="text-button" onClick={()=>{setLens('published');setReveal(false);setSeed(20260909)}}><RotateCcw size={12}/> Reset</button></div>
        </section>
        <aside className="target-inspector">
          <div className="panel-heading"><span>UNDER THE LENS</span><span className="inspector-coordinate">PT {cleanPatient(patient)} / {selected.number}</span></div>
          <AnimatePresence mode="wait"><motion.div key={selected.id} initial={{opacity:0,y:5}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.12}}>
            ...
            <h2 className="gene-name">{selected.gene}</h2><div className="mutation-name">{selected.mutation??'Complex variant'} <span>PROTEIN CHANGE</span></div>
            <div className="inspector-section"><span className="micro-heading">VACCINE TARGET SEQUENCE</span><Sequence target={selected}/><div className="sequence-key"><i/> changed from normal sequence</div></div>
            ...
            {selected.proteinMapping?.status==='verified'?<div className="mapping-state"><Check size={12}/> Verified protein context</div>:<div className="mapping-state unresolved"><CircleHelp size={12}/>{selected.proteinMapping?.reason??'Reference mapping pending'}</div>}
            {reveal&&['pooled','missing'].includes(selected.outcome)&&<p className="record-note">{selected.outcome==='pooled'?'Member of a responding pool. Its individual response is unresolved.':'No assay outcome is available for this target.'} Excluded from response-ranking analysis.</p>}
          </motion.div></AnimatePresence>
          {lens!=='published'&&reveal&&<ChanceChart targets={patientTargets} visible={reveal}/>}
          <button className="molecule-link" onClick={()=>setScene('molecule')}><div className="mini-orbit"><Atom size={26}/></div><span>What does recognition<br/><b>look like?</b></span><ArrowUpRight size={19}/></button>
        </aside>
      </div>
      <footer className="page-footer"><span>ROJAS ET AL. / NATURE 2023</span><span>Observations, predictions, and their limits.</span><button onClick={()=>setScene('molecule')}>Next: a molecular encounter <ArrowRight size={14}/></button></footer>
