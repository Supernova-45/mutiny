...
export function orderTargets(targets, lens, seed = 20260909) {
  ...
  return [
    ...eligible.sort((a, b) => a[lens] - b[lens] || a.id.localeCompare(b.id)),
    ...rest,
  ];
}
...
export function chanceEnvelope(items, seed = 20260909, replicates = 2000) {
  ...
  const curves = Array.from({ length: replicates }, (_, i) =>
    cumulative(shuffle(canonical, seed + i)),
  );
  return Array.from({ length: items.length + 1 }, (_, k) => {
    const values = curves.map((c) => c[k]).sort((a, b) => a - b);
    return {
      low: values[Math.floor((replicates - 1) * 0.05)],
      high: values[Math.floor((replicates - 1) * 0.95)],
      mean: values.reduce((a, b) => a + b, 0) / replicates,
    };
  });
}
