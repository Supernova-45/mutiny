...
- A concise list of supported scientific conclusions, with claim IDs pointing into `evidence.json`. This is evidence documentation, not a narration script.
- Pushed GitHub updates and the final commit SHA; a patch or ZIP only if pushing is unavailable. No credentials or private chat history.
...
After the other agent has updated GitHub, the user will return to the original account for integration and video creation. The original agent will review the changes, finish the interactions, and generate the video, narration and closed captions from the updated app. `outputs/DEMO_PLAN.md` is reserved for that later work and is not an assignment for the Rosalind-enabled agent. It will retain the two studies as separate evidence, verify the files, and only then update the Rosalind attribution. Actual Workbench screenshots and exact plugins are needed for the showcase. No social submission or deployment is authorized by this handoff.
...
Use `outputs/RESEARCH_APP_SPEC.md` as the baseline implementation contract. Resolve its scientific semantics explicitly and record any proposed deviations in `rosalind/PRODUCT_REVIEW.md`; do not silently change the analysis unit or scope. The schema version is a proposal until validated.
...
Additional schema work: supply `rosalind/evidence-request.schema.json` and a matched request/return example with a project fingerprint, exact selected-candidate joins, at least one unresolved result, and one separately represented proposed correction. Define how duplicate imports, conflicting mappings and version mismatches are handled.

Additional scientific validation: cover duplicate assay outcomes across peptide–HLA rows, pooled positivity, missing scores, score direction/units, ties and distinct sequence roles. The Rojas mutant flank must not be mislabeled as its assayed epitope. Return `rosalind/CONTRACT_VALIDATION.md` with results and executable checks. A schema alone does not validate these biological joins.
