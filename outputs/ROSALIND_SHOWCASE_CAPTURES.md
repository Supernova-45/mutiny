...
**For the immersive 3D sequence, also request a wide companion to the HHAT close-up:** `01b-workbench-hhat-receptor.png`, showing peptide, HLA groove and receptor binding domains together. Reopen the prepared 6UK4 receptor complex used by [the native interface render](../rosalind/plugin-results/hhat-native-interface.png); preserve its recorded crystal-mate reconstruction and coordinate frame. Pull the camera back instead of moving protein chains. Keep the peptide visually distinct. The original account can cut from this genuine Workbench view to mutiny's new molecular-context → residue-detail camera transition. A matching clean native render is welcome, with its render sidecar and separate capture-type label.
