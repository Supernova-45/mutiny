...
## Deposited structure reopened and validation applied

On explicit user request, a new native viewer opened the original deposited 6UK4 PDB. Its default assembly 1 contains HLA/peptide chains A/B/C, not the separately reconstructed receptor mate. wwPDB validation now loaded successfully. The returned peptide residue-level RSCC values include W6=0.971 and F8=0.959. These are source-reported whole-residue density-fit correlations, not a new refinement, side-chain-only support score, or certainty estimate. Exact author identities, source URL/hash and plugin response are in `plugin-results/hhat-6uk4-native-validation.json`.

The interactive viewer labels W6 and F8. `hhat-native-validation.png` is its visually inspected native render with a hash-verified, publicly redacted provenance sidecar. It shows the peptide/HLA environment without receptor reconstruction and contains no density overlay. Native public-density loading failed because active workspace-read roots were not bound to the session, although source-relative image publication worked. No alternate path was used to bypass that missing capability. Computer Use access to com.openai.codex was retried and again explicitly denied.
